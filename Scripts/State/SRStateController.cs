...
public abstract class SRStateController<TState, TTrigger> : SRMonoBehaviour, IHasStateMachine<TState, TTrigger>
{
	...
	protected class InternalStateMachine : Stateless.StateMachine<TState, TTrigger>
	{
		...
		public InternalStateMachine(Func<TState> stateAccessor, Action<TState> stateMutator) : base(stateAccessor, stateMutator) {}
	...
	public event StateChangedHandler StateChanged {
		...
		add {
			if(_eventListeners == null) _eventListeners = new SRList<StateChangedHandler>(1);
	...
	protected float StateTime { get { return Time.realtimeSinceStartup - _transTime; } }

	protected virtual TState DefaultState { get { return default (TState); } }
	...
	void OnGUI()
	{
		...
		GUILayout.Label("State: {0} ({1:0.00}s, Last Transition: {2} -({3})> {0})".Fmt(State, StateTime, _prevState, _prevTrigger));
		GUILayout.Label(
